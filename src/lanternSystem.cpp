    ...
    : instanceCount_(count)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        inst.position      = glm::vec3(randF(-spread, spread),
                                       randF(-5.0f, 40.0f),
                                       randF(-spread, spread));
...
void LanternSystem::setupGeometry() {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 instanceCount_ * static_cast<int>(stride),
                 nullptr,
                 GL_DYNAMIC_DRAW);
...
void LanternSystem::setupTexture() {
    ...
    for (int y = 0; y < size; ++y) {
        for (int x = 0; x < size; ++x) {
            ...
            if (border)          { r = 80;  g = 40;  b = 20; }
            else if (vStripe || hBand) { r = 139; g = 69;  b = 19; }
...
void LanternSystem::update(float deltaTime) {
    ...
    for (auto& inst : instances_) {
        ...
        inst.position.x += std::sin(inst.position.y * 2.0f + inst.driftPhase)
                           * inst.driftAmplitude;
        inst.position.z += std::cos(inst.position.y * 2.0f + inst.driftPhase)
                           * inst.driftAmplitude;
...
void LanternSystem::render(const glm::mat4& view,
